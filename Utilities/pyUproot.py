...
class GenericHist:
    def __init__(self, *args):
        ...
        if isinstance(hist, TGraphAsymmErrors):
            ...
            self.underflow = np.array([scale*hist.underflows, scale**2*hist.fSumw2[0]])
            self.overflow = np.array([scale*hist.overflows, scale**2*hist.fSumw2[-1]])
    ...
    def rebin(self, rebin):
        ...
        self.histErr2 = np.sum(self.histErr2.reshape(rebin, size/rebin), axis=1)
